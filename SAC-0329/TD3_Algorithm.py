...
class Agent:
    def __init__(self, alpha, beta, tau, env, input_dims, n_actions, layer1_size,
                 ...
                 warmup=2000, max_size=1000000, batch_size=100, 
                 noise=0.1, checkpoint_dir='TD3_model'):
        self.gamma = gamma
        self.tau = tau
    ...
    def choose_action(self, observation):
        if self.time_step_counter < self.warmup:
            ...
            mu = np.random.normal(scale=0.1, size=7) 
        ...
        # Create exploration noise, in interval [-0.05, 0.05] degree per sec
        self.action_noise = np.random.normal(scale=0.05, size=self.n_actions)
                                #  np.clip(   -self.noise, self.noise, dtype=np.float32)
    ...
    def learn(self):
        ...
        target_actions_noise = np.clip(np.random.normal(scale=0.08, size=(self.batch_size, self.n_actions)),
                                       -0.08, 0.08, dtype=np.float32)
