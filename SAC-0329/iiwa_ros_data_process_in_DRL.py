...
class DataProcessing(object):
    def __init__(self, file_name):
        ...
        self.config_df = pd.DataFrame(columns=self.joint_index)
        self.config_num = 0

        self.path_joint_space_writer = pd.ExcelWriter(self.path_joint_space_file)
        self.path_joint_space_df = pd.DataFrame(columns=self.joint_index)
    ...
    def add_path_joint_space(self, config, step):
        config_dict = {'J1':config[0], 'J2':config[1], 'J3':config[2], 'J4':config[3], 
                       'J5':config[4], 'J6':config[5], 'J7':config[6]}
        self.path_joint_space_df.loc[step] = pd.Series(config_dict)

    def add_config_df(self, config):
        config_dict = {'J1':config[0], 'J2':config[1], 'J3':config[2], 'J4':config[3], 
                       'J5':config[4], 'J6':config[5], 'J7':config[6]}
        self.config_df.loc[self.config_num] = pd.Series(config_dict)
        self.config_num += 1
