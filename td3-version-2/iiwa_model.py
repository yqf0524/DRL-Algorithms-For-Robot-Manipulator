...
class KinematicModel(object):
    ...
    def __init__(self):
        ...
        self.start_ee_se3 = np.zeros((4,4))
        self.start_ee_xyz = np.zeros(3)
        self.start_ee_rpy = np.zeros(3)
        self.start_ee_pose = np.zeros(6)
    ...
    def set_init_configuration(self, init_configuration):
        ...
        self.update_kinematic()
        self.start_ee_se3 = self.current_ee_se3
        self.start_ee_xyz = self.current_ee_xyz
        self.start_ee_rpy = self.current_ee_rpy
        self.start_ee_pose = self.current_ee_pose
    ...
    def is_in_workspace(self):
        ...
        self.in_workspace = ((0.3 <= ee_xyz[0] and ee_xyz[0] <= 1.0) and \
                            (-0.5 <= ee_xyz[1] and ee_xyz[1] <= 0.5) and \
                            (0.01 <= ee_xyz[2] and ee_xyz[2] <= 0.7))
    ...
    def is_achieved_goal(self):
        ...
        rpy_error_1 = self.current_ee_rpy - self.target_ee_rpy
        rpy_error_2 = self.current_ee_rpy + self.target_ee_rpy
