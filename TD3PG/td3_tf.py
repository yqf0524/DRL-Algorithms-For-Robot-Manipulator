...
class Agent():
    def __init__(self, alpha, beta, input_dims, tau, env, layer1_size,
                 ...
                 update_actor_interval=2, warmup=1000, n_actions=7,
                 max_size=1000000, batch_size=100):
        self.gamma = gamma
        self.tau = tau
        ...
        self.noise = OrnsteinUhlenbeckNoise(self.n_actions)
        self.update_network_parameters(tau=1)

    def choose_action(self, observation):

        state = tf.convert_to_tensor([observation], dtype=tf.float32)
        mu = self.actor(state)  # returns a batch size of 1, want a scalar array
        noise = self.noise() * 0.0005  # 0.029 degree
        mu_prime = mu + tf.convert_to_tensor([noise], dtype=tf.float32)
    ...
    def learn(self):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            target_actions = target_actions + \
                             tf.clip_by_value(np.random.normal(scale=0.0002), -0.0005, 0.0005)
